...
class Evaluator:
    def __init__(self, eval_dict):
        self.name = eval_dict['task_name']

        self.num_tasks = int(eval_dict['num_tasks'])
        self.eval_metric = eval_dict['eval_metric']
    ...
    def _parse_and_check_input(self, input_dict):
        if self.eval_metric == 'rocauc' or self.eval_metric == 'ap' or self.eval_metric == 'rmse' or self.eval_metric == 'acc' or self.eval_metric == 'reg':
    ...
    def eval(self, input_dict):
        ...
        elif self.eval_metric == 'reg':
            y_true, y_pred = self._parse_and_check_input(input_dict)
            return self._eval_reg(y_true, y_pred)
        else:
            raise ValueError('Undefined eval metric %s ' % (self.eval_metric))
    ...
    def _eval_reg(self,y_true,y_pred):
        mae_list = []
        rmse_list = []
        r2_list = []
        ...
        for i in range(y_true.shape[1]):
            ...
            r2_list.append(1 - np.sum((y_true[is_labeled,i] - y_pred[is_labeled,i])**2) / np.sum((y_true[is_labeled,i] - np.mean(y_true[is_labeled,i]))**2))

        return {'mae': sum(mae_list)/len(mae_list),'rmse': sum(rmse_list)/len(rmse_list),'r2':sum(r2_list)/len(r2_list)}
