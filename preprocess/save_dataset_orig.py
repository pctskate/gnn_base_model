...
"""
@File     : save_dataset_orig.py
@Project  : gnn_base_model
@Time     : 2022/12/8 17:17
@Author   : Pu Chengtao
@Contact_2: 2319189860@qq.com
@Software : PyCharm
@Last Modify Time      @Version     @Desciption
--------------------       --------        -----------
2022/12/8 17:17        1.0             None
"""
import torch
import pandas as pd
import os
...
class DatasetSaver(object):
    ...
    def __init__(self, dataset_name, is_hetero, version, root='submission'):
        ...
        if self.dataset_prefix == 'ogbg' and self.is_hetero:
            raise NotImplementedError(
                'Heterogeneous graph dataset object has not been implemented for graph property prediction yet.')

        if osp.exists(self.dataset_dir):
            if input(
                    f'Found an existing submission directory at {self.submission_dir}/. \nWill you remove it? (y/N)\n').lower() == 'y':
    ...
    def _save_graph_list_hetero(self, graph_list):
        ...
        for ent_type in ent_type_list:
            num_nodes_dict[ent_type] = np.array([graph['num_nodes_dict'][ent_type] for graph in graph_list]).astype(
                np.int64)
                ...
                if value0 is not None:
                    for key1, value1 in value0.items():
                        if isinstance(graph_list[i][key0][key1], np.ndarray):
                            assert (np.allclose(graph_list[i][key0][key1], graph_list_read[i][key0][key1], rtol=1e-04,
    ...
    def save_target_labels(self, target_labels):
        ...
        if self.is_hetero:
            ...
            if len(target_labels[key]) != self.num_data[key]:
                raise RuntimeError(
                    f'The length of target_labels ({len(target_labels[key])}) must be the same as the number of data points ({self.num_data[key]}).')
    ...
    def get_meta_dict(self):
        ...
        meta_dict['url'] = f'https://snap.stanford.edu/ogb/data/{map_dict[self.dataset_prefix]}/' + meta_dict[
            'download_name'] + '.zip'
        meta_dict['add_inverse_edge'] = 'False'
...
def test_datasetsaver():
    ...
    saver.save_task_info(dataset.task_type, dataset.eval_metric,
